<x-app-layout>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select id="document_type" name="document_type" required
                                    ...
                                    <option value="DNI" @selected(old('document_type', $client->document_type) == 'DNI')>DNI</option>
                                    <option value="RUC" @selected(old('document_type', $client->document_type) == 'RUC')>RUC</option>
                                    <option value="PAS" @selected(old('document_type', $client->document_type) == 'PAS')>Pasaporte</option>
                                    <option value="CEX" @selected(old('document_type', $client->document_type) == 'CEX')>Carné de Extranjería</option>
                                </select>
                            ...
                            <div>
                                ...
                                <input type="text" id="document_number" name="document_number" value="{{ old('document_number', $client->document_number) }}" required
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <div class="md:col-span-2">
                                <label for="business_name" class="block text-sm font-medium text-gray-700">Razón Social / Nombre Completo *</label>
                            ...
                            <div>
                                ...
                                <select id="departamento" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    <option value="">Seleccionar...</option>
                                    @foreach ($departamentos as $depto)
                                        <option value="{{ $depto }}" @selected($client->ubigeo?->departamento == $depto)>{{ $depto }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select id="provincia" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        @disabled(!$client->ubigeo)>
                                    <option value="">Seleccionar...</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select id="distrito" name="ubigeo_code" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        @disabled(!$client->ubigeo)>
                                    <option value="">Seleccionar...</option>
                                </select>
                            ...
                            <div>
                                ...
                                <textarea id="address" name="address" rows="2"
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('address', $client->address) }}</textarea>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" id="mobile" name="mobile" value="{{ old('mobile', $client->mobile) }}"
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            </div>
                            ...
                            <div>
                                ...
                                <select id="establishment_id" name="establishment_id" required
                                    ...
                                    @foreach ($establishments as $est)
                                        <option value="{{ $est->id }}" @selected(old('establishment_id', $client->establishment_id) == $est->id)>{{ $est->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="md:col-span-2 flex items-center gap-2">
                                <input type="checkbox" id="is_insurance_company" name="is_insurance_company" value="1"
                                       ...
                                       @checked(old('is_insurance_company', $client->is_insurance_company))>
                            ...
                            <div id="insurance-fields" class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 hidden">
                                <div>
                                    ...
                                    <input type="number" step="0.01" min="0" id="insurance_hourly_rate" name="insurance_hourly_rate"
                                           value="{{ old('insurance_hourly_rate', $client->insurance_hourly_rate) }}"
                                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                </div>
                                <div>
                                    ...
                                    <input type="number" step="0.01" min="0" id="insurance_panel_rate" name="insurance_panel_rate"
                                           value="{{ old('insurance_panel_rate', $client->insurance_panel_rate) }}"
                                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        const clienteUbigeo = @json($client->ubigeo ? [
            'departamento' => $client->ubigeo->departamento,
            'provincia' => $client->ubigeo->provincia,
            'code' => $client->ubigeo->code,
            'distrito' => $client->ubigeo->distrito,
        ] : null);

        departamentoEl.addEventListener('change', function() {
            provinciaEl.disabled = !this.value;
        ...
        provinciaEl.addEventListener('change', function() {
            distritoEl.disabled = !this.value || !departamentoEl.value;
            distritoEl.innerHTML = '<option value="">Seleccionar...</option>';

            if (!this.value) return;

            fetch(`{{ route('api.ubigeo.distritos') }}?departamento=${encodeURIComponent(departamentoEl.value)}&provincia=${encodeURIComponent(this.value)}`)
                .then(r => r.json())
                .then(data => {
                    data.forEach(d => {
                        const opt = document.createElement('option');
        ...
        // Precargar provincia y distrito si el cliente tiene ubigeo
        if (clienteUbigeo) {
            departamentoEl.value = clienteUbigeo.departamento;
            departamentoEl.dispatchEvent(new Event('change'));

            // Al cargar provincias, seleccionar la actual
            window.addEventListener('load', function cargarProvincias() {
                const wait = setInterval(() => {
                    const hasOptions = provinciaEl.options.length > 1;
                    if (hasOptions) {
                        clearInterval(wait);
                        provinciaEl.value = clienteUbigeo.provincia;
                        provinciaEl.dispatchEvent(new Event('change'));

                        const waitDistritos = setInterval(() => {
                            const hasDistritos = distritoEl.options.length > 1;
                            if (hasDistritos) {
                                clearInterval(waitDistritos);
                                distritoEl.value = clienteUbigeo.code;
                            }
                        }, 100);
                    }
                }, 100);
