<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap justify-between items-center gap-3">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Costos y Utilidad por OT</h2>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <form method="GET" action="{{ route('reports.profitability') }}" class="card p-4 mb-4">
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 items-end">
                    ...
                    <div class="flex items-center gap-2 col-span-2 md:col-span-1">
                        ...
                        <a href="{{ route('reports.profitability') }}" class="btn btn-secondary">Limpiar</a>
                    </div>
                </div>
            </form>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
                <div class="card p-4 sm:p-5 lg:col-span-2">
                    <h3 class="mb-2 text-sm font-semibold text-gray-700">Ingresos, costos y utilidad por mes</h3>
                    <div id="chart-monthly"></div>
                </div>
                <div class="card p-4 sm:p-5">
                    <h3 class="mb-2 text-sm font-semibold text-gray-700">Distribución de costos</h3>
                    <div id="chart-cost"></div>
                </div>
                <div class="card p-4 sm:p-5 lg:col-span-3">
                    <h3 class="mb-2 text-sm font-semibold text-gray-700">Órdenes de trabajo por estado</h3>
    ...
    @push('scripts')
        ...
        <script>
            const table = new Tabulator('#report-table', {
                ...
                height: 'auto',
                placeholder: 'No hay órdenes de trabajo para los filtros seleccionados',
                pagination: true,
                paginationSize: 10,
                paginationSizeSelector: [10, 25, 50],
                columns: [
                    {
                        title: 'OT', field: 'document_sn', width: 130,
                        formatter: (cell) => `<a href="/work-orders/${cell.getData().id}" class="font-medium text-blue-600 hover:text-blue-700 hover:underline">${cell.getValue()}</a>`,
                    },
                    { title: 'Fecha', field: 'date', width: 95, hozAlign: 'center' },
                    { title: 'Placa', field: 'plate', width: 90 },
                    {
                        title: 'Vehículo', field: 'model',
                        formatter: (cell) => `${cell.getData().brand} ${cell.getValue()}`.trim(),
                    },
                    { title: 'Cliente', field: 'client' },
                    { title: 'Ingreso', field: 'income', width: 100, hozAlign: 'right', sorter: 'number', formatter: (cell) => money(cell.getValue()) },
                    { title: 'Costo', field: 'cost', width: 100, hozAlign: 'right', sorter: 'number', formatter: (cell) => money(cell.getValue()) },
                    {
                        title: 'Utilidad', field: 'profit', width: 100, hozAlign: 'right', sorter: 'number',
                        formatter: (cell) => {
                            const v = Number(cell.getValue() || 0);
                            return `<span class="${v < 0 ? 'text-red-600' : 'text-green-700'}">${money(v)}</span>`;
                        },
                    },
                    { title: 'Margen', field: 'margin', width: 85, hozAlign: 'right', sorter: 'number', formatter: (cell) => pct(cell.getValue()) },
                    {
                        title: 'Estado', field: 'status_label', width: 150,
                        formatter: (cell) => `<span class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold bg-gray-100 text-gray-700">${cell.getValue()}</span>`,
                    },
                ],
            });

            async function loadReport() {
                const res = await fetch("{{ route('api.reports.profitability') }}?" + buildQuery(), { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
                ...
                renderChart('chart-monthly', Object.assign(baseLine(), {
                    series: [
                        { name: 'Ingresos', data: data.series.monthly.map(m => m.income) },
                        { name: 'Costos', data: data.series.monthly.map(m => m.cost) },
                        { name: 'Utilidad', data: data.series.monthly.map(m => m.profit) },
                    ],
                ...
                renderChart('chart-cost', Object.assign(baseDonut(), {
                    labels: data.series.cost_donut.map(c => c.name),
                    series: data.series.cost_donut.map(c => c.value),
                    tooltip: { theme: 'light', y: { formatter: (v) => 'S/ ' + Number(v || 0).toLocaleString('es-PE') } },
                }));
                renderChart('chart-status', Object.assign(baseBar(), {
                    colors: ['#2563eb'],
                    series: [{ name: 'OTs', data: data.series.status.map(s => s.count) }],
                    xaxis: { categories: data.series.status.map(s => s.name) },
                }));
            ...
            document.getElementById('btn-export').addEventListener('click', () => table.download('csv', 'costos-utilidad.csv'));
