<x-app-layout>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select id="client_id" name="client_id" required
                                    ...
                                    <option value="{{ $vehicle->client_id }}" selected>{{ $vehicle->client?->business_name }}</option>
                                </select>
                        ...
                        <div class="mt-8">
                            ...
                            <div id="contacts-container" class="space-y-4">
                                @foreach($vehicle->contacts as $contact)
                                    <div class="p-4 border border-gray-200 rounded-lg bg-gray-50 contact-item">
                                        <div class="flex justify-between items-center mb-3">
                                            <span class="text-sm font-semibold text-gray-700">Contacto existente</span>
                                            <button type="button" class="remove-contact text-red-600 hover:text-red-800 text-sm">Eliminar</button>
                                        </div>
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700">Tipo *</label>
                                                <select name="contacts[{{ $loop->iteration }}][type]" required
                                                        class="contact-type mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                                    <option value="">Seleccionar...</option>
                                                    <option value="approver" @selected($contact->type == 'approver')>Aprobador</option>
                                                    <option value="driver" @selected($contact->type == 'driver')>Chofer</option>
                                                    <option value="operator" @selected($contact->type == 'operator')>Operador</option>
                                                </select>
                                            </div>
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700">Nombre *</label>
                                                <input type="text" name="contacts[{{ $loop->iteration }}][name]" value="{{ $contact->name }}" required
                                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                            </div>
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700">Teléfono</label>
                                                <input type="text" name="contacts[{{ $loop->iteration }}][phone]" value="{{ $contact->phone }}"
                                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                            </div>
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700">Email</label>
                                                <input type="email" name="contacts[{{ $loop->iteration }}][email]" value="{{ $contact->email }}"
                                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                            </div>
                                            <div class="company-field {{ $contact->type == 'operator' ? '' : 'hidden' }}">
                                                <label class="block text-sm font-medium text-gray-700">Empresa (solo operador)</label>
                                                <input type="text" name="contacts[{{ $loop->iteration }}][company_name]" value="{{ $contact->company_name }}"
                                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                            </div>
                                        </div>
                                    </div>
    ...
    @push('scripts')
    <script>
        // Tom Select para seleccionar cliente (con opción precargada)
        const clientTomSelect = new TomSelect('#client_id', {
            valueField: 'id',
        ...
        // Contactos dinámicos
        let contactCount = document.querySelectorAll('#contacts-container .contact-item').length;
        const maxContacts = 3;
        ...
        addContactBtn.addEventListener('click', () => {
            ...
            div.className = 'p-4 border border-gray-200 rounded-lg bg-gray-50 contact-item';
            div.innerHTML = `
                <div class="flex justify-between items-center mb-3">
                    <span class="text-sm font-semibold text-gray-700">Contacto nuevo</span>
                    <button type="button" class="remove-contact text-red-600 hover:text-red-800 text-sm">Eliminar</button>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
            ...
            const typeSelect = div.querySelector('select.contact-type');
        ...
        // Enlazar el cambio de tipo a los contactos existentes
        document.querySelectorAll('#contacts-container .contact-item').forEach(item => {
            const select = item.querySelector('select.contact-type');
            const companyField = item.querySelector('.company-field');
            if (select) {
                select.addEventListener('change', () => {
                    companyField.classList.toggle('hidden', select.value !== 'operator');
                    if (select.value !== 'operator') {
                        companyField.querySelector('input').value = '';
                    }
                });
                // Ocultar empresa si el tipo no es operador
                if (select.value !== 'operator') {
                    companyField.classList.add('hidden');
                }
            }

            item.querySelector('.remove-contact').addEventListener('click', () => {
                item.remove();
                contactCount--;
                renderContacts();
            });
